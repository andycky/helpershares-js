...
const API_NAME_HELPERDATA_BY_EM="helperDatabyEmployer";
...
const Employers: NextPage = () => {
      ...
      const fetchData = async (_api:string,helper:helperData) => {
        if(_api==API_NAME_HELPERDATA_BY_EM){
          ...
          const newData = await req.text();
        }else if(_api==API_NAME_ADD_HP_HELPER){
          const req = await fetch(address+"?api="+_api+"&link="+helper.link);
          const newData = await req.text();
      ...
      useEffect(() => { 
          setIsLoading(true);
          // 使用 AJAX 异步请求数据
          axios.get(address+"?api="+API_NAME_EMPLOYERDATA).then(response => {
                  ...
                  <AccordionDetails>
                    <Box sx={{ flexGrow: 1 }}>
                      <Grid container spacing={3}>
                        ...
                        <Grid xs={2}>
                          <Button onClick={(event) => handleClick(event,API_NAME_HELPERDATA_BY_EM,JSON.parse("{\"employer\":\""+employer.name+"\"}"))}>
